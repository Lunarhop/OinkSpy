...
void setupServer() {
    ...
    gServer.on("/api/stats", HTTP_GET, [](AsyncWebServerRequest* request) {
        ...
        snprintf(buf,
                 sizeof(buf),
                 "{\"total\":%d,\"raven\":%d,\"ble\":\"active\",\"gps_valid\":%s,\"gps_age\":%lu,\"gps_tagged\":%d,\"sd_ready\":%s,\"sd_logging_healthy\":%s,\"time_synced\":%s,\"time_source\":\"%s\",\"day_token\":\"%s\",\"scan_enabled\":%s,\"companion_active\":%s}",
                 gApp.detectionCount,
